...
module HistoricalDiary
  module JekyllLayer
    # Wrapper combining a `sources` Data File with a <tt>SourceDocument</tt>.
    class SourceDrop < Jekyll::Drops::Drop
      ...
      def language = record['name_language'] || record['language']
      ...
      def citation_data
        {
          ...
          'date' => self['date'],
          'editor' => self['editor_key'],
          'publisher' => self['publisher'],
          'title' => presentational_name,
          'year' => self['date']&.scan(/\A\d{4}/)&.first,
        }.compact
      end
